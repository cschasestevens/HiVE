#' Lipid Network Multimodal Data Plot
#'
#' Extension of hive_base() for plotting multimodal abundance/expression
...
#' of HiVE base.
#' @param obj_lipidome A data frame of statistical results containing
#' fold changes and p-values or a Seurat object. The labels must match
#' nodes from the HiVE base network nodes to properly function.
#' @param obj_transcriptome Seurat object or DGEA results to plot.
...
#' @param col_ct Cell type column name in transcriptomics data.
...
#' @examples
#'
#' # ## Multimodal data network
#' # hive_mult(obj_lipidome = d1, obj_transcriptome = d2)
#'
#' @export
hive_mult <- function( # nolint
  type = "oxylipins",
  obj_lipidome,
  obj_transcriptome,
  col_fc = "Log2FC",
  ...
  # Load lipidomics/transcriptomics objects
  net_lip <- obj_lipidome
  net_gex <- obj_transcriptome
  ...
  if (gex_type == "GEX") {
    ...
    net_gex <- SeuratObject::FetchData(
      net_gex,
      vars = c(
        col_ct,
        net_genes[net_genes %in% rownames(net_gex)]
      )
    )
    if (col_ct != "CellType") {
      net_gex[["CellType"]] <- net_gex[[col_ct]]
      net_gex <- dplyr::select(
        net_gex,
        -c(col_ct),
    ...
    net_gex[[1]] <- factor(
      as.character(net_gex[[1]]),
      levels = gtools::mixedsort(unique(as.character(net_gex[[1]])))
    )
    net_gex <- as.data.frame(scale(
      as.matrix(
  ...
  if (gex_type == "DGEA") {
    ...
    net_gex <- net_gex[
      net_gex[["GENE"]] %in% net_genes,
      c(col_ct, "GENE", "log2FC")
    ]
    # Convert cell type to factor and format as data frame
    net_gex[[col_ct]] <- factor(
      as.character(net_gex[[col_ct]]),
      levels = gtools::mixedsort(unique(as.character(net_gex[[col_ct]])))
    )
    if (col_ct != "CellType") {
      net_gex[["CellType"]] <- net_gex[[col_ct]]
      net_gex <- dplyr::select(net_gex, -c(col_ct))
    }
    net_gex <- reshape2::dcast(
      net_gex, CellType ~ GENE, value.var = "log2FC"
    )
    net_gex[is.na(net_gex)] <- 0
    net_gex <- dplyr::select(
      magrittr::set_rownames(
        net_gex, levels(net_gex[["CellType"]])
      ),
      -c("CellType")
    )
  }
      ...
      d_out <- tryCatch(
        {
          ...
          np1 <- net_lip[net_lip[[col_lab]] == nn[["Node"]][[i]], ]
          # Check synonyms column to verify that no matches exist
          if (nrow(np1) == 0) {
            np1 <- net_lip[net_lip[[col_lab]] == nn[["Synonyms"]][[i]], ]
          }
          if (nrow(np1) == 0) {
            np1 <- paste(
      ...
      d_out <- tryCatch(
        {
          ...
          if (length(np_match[np_match %in% names(net_gex)]) == 1) {
            np1 <- magrittr::set_rownames(
              setNames(
                as.data.frame(np1),
                np_match[np_match %in% names(net_gex)]
              ),
              rownames(net_gex)
            )
          }
