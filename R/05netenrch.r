...
#' object.
#' @param col_sat Column name that indicates the saturation degree
#' of each lipid class included in the input enrichment object. Use the
#' same column for both col_lab and col_sat if compound classes split
#' by saturation degree are not available.
#'
...
hive_enrch <- function( # nolint
  ...
  net1[["label.saturation"]] <- net1[[col_sat]]
  #---- Enrichment network ----
  ...
  if (
    length(
      grepl("_Sat|_Unsat", names(npl))[
        grepl("_Sat|_Unsat", names(npl)) == TRUE
      ]
    ) == 0
  ) {
    np_nodes <- setNames(
      lapply(
        ...
        function(h) {
          dnode <- data.frame(
            "x" = npl[["x"]],
            "y" = npl[["y"]],
            "size" = rep(0, length(npl[["x"]])),
            "fill" = rep(0.5, length(npl[["x"]])),
            "color" = rep("b", length(npl[["x"]])),
            "lab" = rep("", length(npl[["x"]])),
            "lab_FDR" = rep("", length(npl[["x"]])),
            "lab_size" = rep(0, length(npl[["x"]]))
          )
